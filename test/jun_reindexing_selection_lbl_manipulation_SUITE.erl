...
test_jun_pandas_drop([{jun_worker, Pid}, {path, Path}, _]) ->
    ...
    ?assertEqual({?DATAFRAME, [<<"name">>], [[<<"Allison">>],
...
test_jun_pandas_rename([{jun_worker, Pid}, {path, Path}, _]) ->
    ...
    ?assertEqual({?DATAFRAME, [<<"name">>, <<"agex">>], [[<<"Allison">>,29],
...
test_jun_pandas_append([{jun_worker, Pid} | _]) ->
    ...
    ?assertEqual({?DATAFRAME, [<<"id">>, <<"name">>], [[1, <<"Tutti">>], [2, <<"Ziggy">>]]}, Erl).
...
test_jun_pandas_update([{jun_worker, Pid} | _]) ->
    ...
    {ok, Erl} = jun_pandas:to_erl(Pid, FinalDataFrame),
    ?assertEqual({?DATAFRAME, [<<"id">>, <<"name">>], [[1, <<"Tutti">>], [2, <<"Ziggyfredo">>]]}, Erl).
...
test_jun_pandas_reset_index([{jun_worker, Pid}, {path, Path} | _]) ->
    ...
    ?assertEqual({?DATAFRAME, [<<"age">>,<<"name">>], [[29,<<"Allison">>],
...
test_jun_pandas_drop_duplicates([{jun_worker, Pid}, {path, Path} | _]) ->
    ...
    {ok, Erl} = jun_pandas:to_erl(Pid, FinalDataFrame),
    ?assertEqual({?DATAFRAME, [<<"name">>,<<"age">>], [[<<"Allison">>, 29],
