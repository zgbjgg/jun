...
def jun_dataframe(df, fn, args, axis='None', keywords=[]):
    if ( isinstance(df, pd.core.frame.DataFrame) | isinstance(df, pd.core.groupby.DataFrameGroupBy) ):
        ...
        if isinstance(value, np.int64): 
            ...
            return (Atom(b'pandas.core.frame.DataFrame'), value)
        elif isinstance(value, pd.core.groupby.DataFrameGroupBy):
            return (Atom(b'pandas.core.groupby.DataFrameGroupBy'), value)
        elif isinstance(value, pd.core.frame.Series):
            return (Atom(b'pandas.core.frame.Series'), value)
        elif isinstance(value, np.ndarray):
            return ','.join(str(v) for v in value)
        elif value is None: # commonly when fun applies over callable object
            return (Atom(b'pandas.core.frame.DataFrame'), df)
        else:
            return value
    else:
...
def to_erl(value):
    if isinstance(value, pd.core.frame.DataFrame):
        ...
        columns = list(value)
        return (Atom(b'pandas.core.frame.DataFrame'), columns, jundataframe)
    else:
...
def jun_dataframe_plot(df, save='None', keywords=[]):
    if ( isinstance(df, pd.core.frame.DataFrame) ):
        ...
        if save != 'None':
            ...
            return (Atom(b'matplotlib.AxesSubplot'), plot) # this is correct? because can be confusing with opaque df
    else:
        return 'error_format_data_frame_invalid'
...
def selection(df, columns):
    if ( isinstance(df, pd.core.frame.DataFrame) ):
        return (Atom(b'pandas.core.frame.DataFrame'), df[list(columns)])
    else:
        return 'error_format_data_frame_invalid'
...
def legacy_query(df, column, operand, value):
    if ( isinstance(df, pd.core.frame.DataFrame) | isinstance(df, pd.core.groupby.DataFrameGroupBy) ):
        ...
        if isinstance(newdf, pd.core.frame.DataFrame):
            return (Atom(b'pandas.core.frame.DataFrame'), newdf)
        elif isinstance(newdf, pd.core.groupby.DataFrameGroupBy):
            return (Atom(b'pandas.core.groupby.DataFrameGroupBy'), newdf)
        else:
            return newdf
    else:
        return 'error_format_data_frame_invalid'
...
def legacy_assignment(df, column, value):
    if ( isinstance(df, pd.core.frame.DataFrame) | isinstance(df, pd.core.groupby.DataFrameGroupBy) ):
        ...
        if isinstance(df, pd.core.frame.DataFrame):
            ...
            return (Atom(b'pandas.core.groupby.DataFrameGroupBy'), df)
        else:
            return df
    else:
        return 'error_format_data_frame_invalid'
...
def single_selection(df, column):
    if ( isinstance(df, pd.core.frame.DataFrame) ):
        return (Atom(b'pandas.core.frame.Series'), df[column])
    else:
        return 'error_format_data_frame_invalid'
